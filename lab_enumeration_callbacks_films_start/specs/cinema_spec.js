...
describe('Cinema', function () {
  ...
  const actual = cinema.findAllFilmsByTitle()
  assert.deepStrictEqual(actual,films)
  });


  it('should be able to find a film by title', function(){
    const actual = cinema.findAFilmByTitle('Moonlight')
  ...
  const actual = cinema.filterByYear(2000)
  assert.deepStrictEqual(actual, "no films, goodbye")
});

  it('should be able to check whether all films are over a particular length', function(){
    const actual = cinema.lengthOfFilm(120)
